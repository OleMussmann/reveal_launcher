...
def refresh_template(running_refreshing):
...
class Handler(FileSystemEventHandler):
    def __init__(self, running_refreshing):
        self.running_refreshing = running_refreshing

    def on_any_event(self, event):
        if event.is_directory==False and \
                (event.src_path.endswith(".md") \
                or event.src_path.endswith(".html") \
                and not event.src_path.endswith("index.html")):
            refresh_template(self.running_refreshing)

def watch(running_watch, running_refreshing):
    print("Running conversion once")
    refresh_template(running_refreshing)
    print("Watching for changes")
    observer = Observer()
    handler = Handler(running_refreshing)
...
def main():
    ...
    watching = threading.Thread(target=watch, args=(running_watch, running_refreshing), daemon=True)
    watching.start()
